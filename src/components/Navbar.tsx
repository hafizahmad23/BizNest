...
interface NavbarProps {
  ...
  /** # of businesses the signed-in user already owns (one-per-account rule). */
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Wraps the mobile controls (bell + sheet) so the outside-click close
  // doesn't fight the bell's own toggle on small screens.
  const mobileControlsRef = useRef<HTMLDivElement>(null);

  const unreadCount = notifications.filter(n => !n.isRead).length;

  // One business per account: owners no longer see "Add Business" anywhere —
  // the CTA becomes a quick link to their existing listing's dashboard.
  const alreadyOwnsBusiness = Boolean(currentUser) && ownedBusinessCount >= 1;

  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      const target = e.target as Node;
      const insideDesktop = notifRef.current?.contains(target);
      const insideMobile = mobileControlsRef.current?.contains(target);
  ...
  /**
   * Shared notification items list — rendered inside BOTH the desktop
   * dropdown (md+) and the mobile full-bleed sheet (<md). Extracted so the
   * two surfaces can never drift apart.
   */
  const renderNotificationList = () => (
    <div className="max-h-[70vh] md:max-h-80 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 overscroll-contain">
      ...
      <div className={`max-w-7xl mx-auto rounded-2xl transition-all duration-300 ${
        ...
        <div className="hidden md:flex items-center gap-2 min-w-0">
          ...
          <div className="relative" ref={notifRef}>
            ...
            {/* Notification Dropdown Panel (desktop / md+). On small screens
                the mobile sheet below renders instead — this absolute dropdown
                used to be clipped/invisible below md, which is why the bell
                "did nothing" on ~360px phones. */}
            {notifOpen && (
              <div className={`absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl border shadow-2xl z-50 overflow-hidden hidden md:block ${
                ...
                {/* Notification List (shared with mobile sheet) */}
          ...
          {/* Primary CTA: Add Business — hidden once the user owns a listing
              (one business per account); owners get a dashboard shortcut. */}
          {alreadyOwnsBusiness ? (
            <button
              onClick={() => onNavigate('dashboard')}
              ...
              title="One business per account — manage your listing"
            >
              <LayoutDashboard className="w-4 h-4" />
        ...
        {/* Mobile menu toggle */}
        <div className="flex md:hidden items-center gap-1.5" ref={mobileControlsRef}>
          <button
            onClick={() => setNotifOpen(!notifOpen)}
          ...
          {/* MOBILE NOTIFICATIONS SHEET (<md) — full-bleed bottom sheet.
              Root cause of the old bug: the notification panel only existed
          ...
          {notifOpen && (
            <div className="fixed inset-0 z-[80] md:hidden" role="dialog" aria-label="Notifications">
              ...
              {/* Bottom sheet */}
              <div
                className={`absolute inset-x-0 bottom-0 max-h-[82vh] rounded-t-3xl border-t shadow-2xl flex flex-col overflow-hidden ${
                  isDarkMode
                    ? 'bg-[#0a0f1d] border-slate-800 text-white'
                    : 'bg-white border-slate-200 text-slate-900'
                }`}
              >
                {/* Grab handle */}
                <div className="pt-2 pb-1 flex justify-center shrink-0">
                  <div className={`w-10 h-1.5 rounded-full ${isDarkMode ? 'bg-slate-700' : 'bg-slate-300'}`} />
                </div>
                ...
                <div className="p-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider truncate">Notifications</span>
                    {unreadCount > 0 && (
                      <span className="px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-400 font-extrabold text-[10px] border border-rose-500/30 shrink-0">
                        {unreadCount} new
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      onClick={() => setNotifOpen(false)}
                      className="p-1.5 rounded-lg bg-slate-800/70 dark:bg-slate-800 text-slate-300 hover:text-white transition cursor-pointer"
                ...
                {/* Shared, scrollable notification list */}
                <div className="flex-1 min-h-0 flex flex-col">
                  {renderNotificationList()}
                </div>

                {/* Footer */}
                <div className="p-2.5 bg-slate-100/80 dark:bg-slate-950 text-[10px] text-center text-slate-400 border-t border-slate-200 dark:border-slate-800 shrink-0">
                  BizNest Real-Time Merchant &amp; Customer Alerts
                </div>
              </div>
            </div>
