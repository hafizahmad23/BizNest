...
import { MapPin, Building2, Compass, ArrowUpRight, Globe, Layers, CheckCircle2 } from 'lucide-react';
...
export const PakistanMap: React.FC<PakistanMapProps> = ({
  ...
  const handleCascadingChange = (loc: LocationSelectionValue) => {
    ...
    const target = loc.cityOrVillageName !== 'All Areas' 
  ...
  return (
    <section className="py-12 px-4 sm:px-8 max-w-7xl mx-auto">
      <div className={`p-6 sm:p-10 rounded-3xl border overflow-hidden relative transition-all ${
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-end justify-between mb-8 gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 text-xs font-bold mb-3">
              ...
              <span>National GPS Coverage Database</span>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-300 mt-1 max-w-2xl">
              Filter businesses across every Province, District, Tehsil, and Village with interactive Google Maps location verification.
            </p>
          </div>
        ...
        {/* Administrative Hierarchy Stats Bar */}
        <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-xs">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-slate-400 font-semibold">Provinces:</span>
            <span className="font-extrabold text-emerald-400">{stats.totalProvinces} Regions</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
            <span className="text-slate-400 font-semibold">Districts:</span>
            <span className="font-extrabold text-cyan-400">{stats.totalDistricts} Districts</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
            <span className="text-slate-400 font-semibold">Tehsils:</span>
            <span className="font-extrabold text-purple-400">{stats.totalTehsils} Tehsils</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
            <span className="text-slate-400 font-semibold">Areas & Villages:</span>
            <span className="font-extrabold text-amber-400">{stats.totalAreasAndVillages}+ Locations</span>
          </div>
        </div>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent p-4 flex flex-col justify-between">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-slate-300 font-medium mt-1">
                        ...
                        <span>{city.businessCount.toLocaleString()}+ Listed Businesses</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
